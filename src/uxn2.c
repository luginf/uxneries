...
static void screen_deo_x(void) { rX = (dev[0x28] << 8) | dev[0x29], rX = (short)rX; }
static void screen_deo_y(void) { rY = (dev[0x2a] << 8) | dev[0x2b], rY = (short)rY; }
...
#define Dei         Pu1(r,emu_dei(x)) if(d) Pu1(r,emu_dei(x+1))
#define Deo         if(d) emu_deo(x,y>>8), emu_deo(x+1,y); else emu_deo(x,y);
...
{
	int i = 1, zoom_presses = 0;
	if(argc == 2 && argv[1][0] == '-' && argv[1][1] == 'v')
		return !fprintf(stdout, "%s - Varvara Emulator, 20 May 2026.\n", argv[0]);
	else if(argc == 1)
		return !fprintf(stdout, "usage: %s [-v | -2x | -3x] file.rom [args..]\n", argv[0]);
	if(i < argc && argv[i][0] == '-' && argv[i][1] == '2' && argv[i][2] == 'x' && !argv[i][3])
		zoom_presses = 1, i++;
	else if(i < argc && argv[i][0] == '-' && argv[i][1] == '3' && argv[i][2] == 'x' && !argv[i][3])
		zoom_presses = 2, i++;
	if(i >= argc || !system_boot(argv[i], argc > i + 1))
		return !fprintf(stdout, "Could not load %s.\n", i < argc ? argv[i] : argv[0]);
	i++;
	screen_resize(WIDTH, HEIGHT);
